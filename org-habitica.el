...
(defun org-habitica--create-task-get-id (caption state &optional notes priority-value attribute type habit-buttons)
  "Create task by TYPE with CAPTION, NOTES, PRIORITY and STATE in Habitica and return its id."
  (org-habitica--get-from-data 'id (org-habitica--format-response-from-buffer (org-habitica--create-task caption state notes priority-value attribute type habit-buttons))))
...
(defun org-habitica--create-set-id-and-score-task (caption state notes &optional priority-value attribute type habit-buttons)
  ...
  (let* ((priority-value (or priority-value (org-habitica--get-priority-value-by-name (car org-habitica-priorities))))
	 (task-id (org-habitica--create-task-get-id caption state notes priority-value attribute type habit-buttons)))
...
(defun org-habitica--create-task (caption state &optional notes priority-value attribute type habit-buttons)
...
(defun org-habitica--update-task (task-id org-state data &optional found-task)
  ...
  (when data
    ...
    (org-habitica--score-task-by-state task-id (org-habitica--get-from-data 'completed found-task) org-state)))
